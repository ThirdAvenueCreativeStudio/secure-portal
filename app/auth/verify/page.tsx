...
function VerifyInner(){
  ...
  useEffect(()=>{
    ...
    .then(r=>r.json())
    .then(data=>{
      if(data.success){localStorage.setItem('user',JSON.stringify(data.user));const role=data.user.role;router.push(role==='admin'?'/admin':role==='bank_admin'?'/bank-admin':role==='officer'?'/officer':'/dashboard');}
