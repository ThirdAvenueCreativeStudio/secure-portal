...
router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const appResult = await pool.query(
      'SELECT id,bank_id FROM applications WHERE applicant_id=$1 ORDER BY created_at DESC LIMIT 1',
      [userId]
    );
    ...
    await pool.query(
      'INSERT INTO audit_log (actor_id,action,entity_type,bank_id,metadata) VALUES ($1,$2,$3,$4,$5)',
      [userId, 'doc.uploaded', 'document', bankId, JSON.stringify({doc_type, size: req.file.size})]
    );
  ...
  } catch(err:any) { console.error('UPLOAD ERR:',err); return res.status(500).json({ error: 'Upload failed', detail: err?.message }); }
});
