...
async function getBillingData(year, month) {
    const start = new Date(year, month - 1, 1);
    const end = new Date(year, month, 1);
    const banks = await db_1.pool.query('SELECT id,name,contact_email FROM banks WHERE active=true');
    const reports = [];
    for (const bank of banks.rows) {
        const c1 = await db_1.pool.query("SELECT COUNT(DISTINCT a.id) as n FROM applications a JOIN users u ON u.id=a.applicant_id WHERE u.bank_id=$1 AND a.status='submitted' AND a.updated_at>=$2 AND a.updated_at<$3", [bank.id, start, end]);
        const c2 = await db_1.pool.query("SELECT COUNT(DISTINCT a.id) as n FROM applications a JOIN users u ON u.id=a.applicant_id WHERE u.bank_id=$1 AND a.status='in_progress'", [bank.id]);
        const c3 = await db_1.pool.query("SELECT COUNT(DISTINCT a.id) as n FROM applications a JOIN users u ON u.id=a.applicant_id WHERE u.bank_id=$1 AND a.status='in_progress' AND a.updated_at<NOW()-INTERVAL '30 days'", [bank.id]);
...
function generateInvoicePDF(bank, month, invoiceNum, applications) {
...
async function sendMonthlyBillingReport(year, month) {
    const reports = await getBillingData(year, month);
    const monthName = new Date(year, month - 1, 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    const totalRevenue = reports.reduce((s, r) => s + r.total, 0);
    // Admin summary HTML
    const rows = reports.map(r => `<tr><td style='padding:8px;border-bottom:1px solid #eee'>${r.bankName}</td>` +
        ...
        `<td style='padding:8px;border-bottom:1px solid #eee;text-align:right;font-weight:bold'>$${r.total.toFixed(2)}</td></tr>`).join('');
    ...
    for (let i = 0; i < reports.length; i++) {
        const r = reports[i];
        if (!r.contactEmail || r.completed === 0)
            continue;
        const num = year + '-' + String(month).padStart(2, '0') + '-' + String(i + 1).padStart(3, '0');
        // Fetch individual applications for this bank this month
        const appRows = await db_1.pool.query("SELECT a.id,u.full_name,u.email,a.updated_at FROM applications a JOIN users u ON u.id=a.applicant_id WHERE u.bank_id=$1 AND a.status='submitted' AND a.updated_at>=$2 AND a.updated_at<$3 ORDER BY a.updated_at", [r.bankId, new Date(year, month - 1, 1), new Date(year, month, 1)]);
    ...
    // Log to audit_log
    await db_1.pool.query("INSERT INTO audit_log (action,entity_type,metadata) VALUES ($1,$2,$3)", ['billing.report_sent', 'billing', JSON.stringify({ month, year, banks: reports.length, total: totalRevenue })]);
